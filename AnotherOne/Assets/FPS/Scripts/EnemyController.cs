...
private bool randomWanderBool = true;
 ...
 IEnumerator Wander()
    {
        ...
        randomWanderBool = false;
 
        yield return new WaitForSeconds(walkWait);
        isWalking = true;

        randomPos = new Vector3(transform.position.x-Random.Range(-20,20),transform.position.y,transform.position.z-Random.Range(-20,20));

        yield return new WaitForSeconds(walkTime);
        isWalking = false;

        randomWanderBool = true;
    ...
    void OnDie()
    {
        ...
        // loot an object
        if (TryDropItem())         //Custom code script in order to specify a loot amount:  They all just spit out right now, need animation code. sin wave?
        {
            foreach(Loot c in lootAmount){     
            Instantiate(c.lootPrefab, transform.position + new Vector3(Random.Range(-0.5f, 0.5f),Random.Range(0.5f, 1.0f),Random.Range(-0.5f, 0.5f)), Quaternion.identity);

            // Set Rigidboy weight so high items drop immediately, and collisions will automatically cause mini-explosion.

            Rigidbody rigid = c.lootPrefab.GetComponent<Rigidbody>();
            rigid.AddForce(5,5,5);

            //rigid.velocity= new Vector3(Random.Range(-10.0f, 10.0f),Random.Range(10.0f, 20.0f),Random.Range(-10.0f, 10.0f));
            }
        }
        
        //doing this after foreach function, to see if it solves any errors
    ...
    {
        foreach(Loot c in lootAmount){     
        if (c.dropRate == 0 || c.lootPrefab == null)
            return false;
        else if (c.dropRate == 1)
            return true;
        else
            return (Random.value <= c.dropRate);
        }
