...
public GameObject healthbarObject;
    ...
    void Start()
    {
        ...
        //healthbarObject = GameObject.Find("/GameManager/GameHUD/HUD/BottomLeftcorner/HorizontalContent/PlayerHealth");
        healthbar = healthbarObject.GetComponent<CanvasGroup>();        
    ...
    void Update()
    {
        ...
        // Regenerate health- check if damage taken, start timer coroutine that checks if no more damage.
...
// Not using- but would be useful for any function that needs to be run for an exact amount of time. 
/*
                    if(!healing) {
...
*/
                ...
                if(healOnce==false && runOnce == false)
                StartCoroutine(Regenerate());

                if(m_Health.takingDamage == false && healOnce == true)
                 m_Health.Heal(0.1f);




                if(m_Health.currentHealth>=m_Health.maxHealth || m_Health.takingDamage == true && healOnce == true) {
                healOnce=false;
                runOnce = false;
                m_Health.takingDamage = false;

                }

                }
                else if(healthbar.alpha != 0)
                healthbar.alpha-=0.01f;
     ...
     IEnumerator Regenerate()
    {
       ...
       //yield return new WaitForSeconds(1);
        ...
        //if(m_Health.currentHealth != lastHealth)
        
        //if(m_Health.currentHealth != m_Health.maxHealth && m_Health.onDamaged != null)
         //m_Health.Heal(0.1f);
...
//Debug.Log("test");
   ...
   //Debug.Log("Recovery TakingDamage = " + m_Health.takingDamage);

        if(!runOnce)
        {

        
  //  Debug.Log("RunOnce = " + runOnce);
        ...
        bool isSprinting = m_InputHandler.GetSprintInputHeld();
        {
            ...
            if (isGrounded)
            {
                ...
                m_footstepDistanceCounter += characterVelocity.magnitude * Time.deltaTime;
                        ...
                        // jumping
                if (jumpCount > 0 && m_InputHandler.GetJumpInputDown())
                {
