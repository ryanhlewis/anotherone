...
public class PlayerWeaponsManager : MonoBehaviour
{
    ...
    public float hBobValue { get; set; }

    public float vBobValue { get; set; }
...
m_WeaponRecoilLocalPosition.z = -0.05f;


m_WeaponRecoilLocalPosition.y = -0.05f;
...
m_AccumulatedRecoil -= Vector3.up * 0.05f;   
m_angleRotation += 1;
}
        ...
        if (activeWeapon && m_WeaponSwitchState == WeaponSwitchState.Up)
        {
            ...
            if (hasFired)
            {
                ...
                m_AccumulatedRecoil += Vector3.up * activeWeapon.recoilForceUp;
    ...
    private void LateUpdate()
    {
        ...
        //rot.eulerAngles = new Vector3(0, 0, m_angleRotation);
        //transform.rotation=rot;
        
        
        weaponParentSocket.localEulerAngles = new Vector3(m_angleRotation, 0, 0);
    ...
    void UpdateWeaponBob()
    {
        if (Time.deltaTime > 0f)
        {
            ...
            //hBobValue = Mathf.Sin(Time.time * frequency) * bobAmount * 0.1f;
            //vBobValue = ((Mathf.Sin(Time.time * frequency * 2f) * 0.5f) + 0.5f) * bobAmount * 0.1f;
            
            
            //idle


             if(characterMovementFactor <= 0.001 && playerCharacterVelocity == Vector3.zero)
            {


            //hBobValue = Mathf.Sin(Time.time ) * 0.01f ;// * bobAmount ;//* 0.01f;
            //vBobValue = ((Mathf.Sin(Time.time *2f )*0.5f)+0.5f) * 0.01f;
            ...
            float hey = percentage * percentage * (3 - 2 *(percentage));
//Debug.Log(percentage);
            ...
            float equation = Mathf.Sin(Time.time ) * 0.01f; 
            //float badequation = (Mathf.Sin(Time.time * frequency) * bobAmount);
 
           // if(Mathf.Round((100 * hBobValue))/100 != Mathf.Round(100 * equation)/100) //{
            //Debug.Log("Before : " + equation);

            // CURRENT PROBLEM. IT THINKS LEFT OF SINE CURVE IS DAMPENING, RIGHT IS NON-DAMPENING
            // Solved, negatives and absolute values.

            //Debug.Log(hBobValue);
            //Debug.Log(equation);

            if(Mathf.Abs(hBobValue) <= Mathf.Abs(equation) * 1.1 || Mathf.Abs(hBobValue) <= Mathf.Abs(equation) * 0.9 )
            {
                
                hBobValue = Mathf.Lerp(0,equation,hey);//percentage);
                
            //{
            //hBobValue = Mathf.SmoothDamp(hBobValue,equation,ref dampener,1.0f,1.0f, Time.deltaTime);
            //Debug.Log("dampening!!");
            }
            else {
             //Debug.Log("not dampening!!");
            hBobValue = equation ;
            }
            //Debug.Log("BOO");
            //hBobValue = Mathf.Lerp(1,2,0.0f);
           ...
           //hBobValue = Mathf.Sin(Time.time ) * 0.01f ;// * bobAmount ;//* 0.01f;
            ...
            //if(Mathf.Round(100 *vBobValue)/100 != Mathf.Round(100* (((Mathf.Sin(tempTime *2f )*0.5f)+0.5f) * 0.01f) )/100) {
            if(vBobValue < (((Mathf.Sin(Time.time *2f )*0.5f)+0.5f) * 0.01f) * 0.9 || vBobValue > 1.1 * (((Mathf.Sin(Time.time *2f )*0.5f)+0.5f) * 0.01f)) 
            vBobValue = Mathf.Lerp(0,((Mathf.Sin(Time.time *2f )*0.5f)+0.5f) * 0.01f,percentage);
             //percentage);
            //vBobValue = Mathf.SmoothDamp(vBobValue,((Mathf.Sin(tempTime *2f )*0.5f)+0.5f) * 0.01f,ref dampener,1.0f,1.0f,Time.deltaTime);
            else
            vBobValue = ((Mathf.Sin(Time.time *2f )*0.5f)+0.5f) * 0.01f;
...
//            Debug.Log("YAY");
            ...
            hBobValue = Mathf.Sin(Time.time * frequency) * bobAmount * m_WeaponBobFactor;
            vBobValue = ((Mathf.Sin(Time.time * frequency * 2f) * 0.5f) + 0.5f) * bobAmount * m_WeaponBobFactor;
            //Debug.Log(m_WeaponBobFactor);
            }
    ...
    void UpdateWeaponRecoil()
    {
        ...
        {
            m_WeaponRecoilLocalPosition.z = Mathf.Lerp(m_WeaponRecoilLocalPosition.z, m_AccumulatedRecoil.z, recoilSharpness * Time.deltaTime);

Debug.Log("Z : " + m_WeaponRecoilLocalPosition.z);
        
        } else if ( m_WeaponRecoilLocalPosition.y >= m_AccumulatedRecoil.y * 0.99f) {
             m_WeaponRecoilLocalPosition.y = Mathf.Lerp(m_WeaponRecoilLocalPosition.y, m_AccumulatedRecoil.y, recoilSharpness * Time.deltaTime);

Debug.Log("Y: " + m_WeaponRecoilLocalPosition.y);

            m_angleRotation = Mathf.Lerp(0, 10, recoilSharpness * Time.deltaTime);
        ...
        else
        {
            ...
            // custom angle
            m_angleRotation = Mathf.Lerp(m_angleRotation,0,Time.deltaTime);
            
            //weaponParentSocket.Rotate(0.0f, m_angleRotation, 0.0f, Space.World);
