...
public int offset = 30;
    ...
    {
		if(centerObject){
		     Rect r=new Rect(Screen.width-10 - RadarSize, 10, RadarSize, RadarSize);
 
		      mapCenter = new Vector2(Screen.width-10-RadarSize/2,10+RadarSize/2);
    ...
    private void drawBlip(GameObject go, Texture aTexture)
    {
      ...
      if (dist <= maxDist)
      {
        ...
        UnityEngine.GUI.DrawTexture(new Rect(mapCenter.x + bX + offset, mapCenter.y + bY + offset, enemyscale, enemyscale), aTexture);
      }
    }
  }